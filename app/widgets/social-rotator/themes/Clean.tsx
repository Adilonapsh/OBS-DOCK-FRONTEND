...
export default function CleanTheme({ socials, index, font, fontSize, accent, textColor, showIcon, showHandle, showLabel, anim }: SocialRotatorThemeProps) {
  ...
  // Tanpa background (langsung di atas video) → wajib text-shadow agar kebaca di scene terang.
  const shadow = { textShadow: READABLE_SHADOW } as const;
  return (
    <div key={`${item.id}-${index}`} className="flex items-center gap-3 will-change-transform" style={{ fontFamily: `'${font}', sans-serif`, animation: `${eff} 0.52s cubic-bezier(0.16,1,0.3,1) both` } as any}>
      ...
      {showLabel && <span className="text-[11px] font-black uppercase tracking-widest" style={{ color: `${color}CC`, fontSize: `${Math.round(fontSize*0.7)}px`, ...shadow }}>{item.label || item.platform}</span>}
      {showHandle && <span className="font-black" style={{ color, fontSize: `${fontSize}px`, ...shadow }}>{item.handle}</span>}
    </div>
  );
}
