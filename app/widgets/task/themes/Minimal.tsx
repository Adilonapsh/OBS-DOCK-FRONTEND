...
export default function MinimalTheme({ tasks, font, onToggleTask, anim, hideAnim, horizontal, inline, exitingIds, collapsedIds, autoCollapse, isExpanded }: TaskThemeProps) {
    ...
    return (
      <div className={`task-minimal-theme ${containerClass} p-2`} style={{ fontFamily: `'${font}', sans-serif` }}>
        {tasks.length === 0 ? (
          ...
          <div className="px-3 py-2 rounded-full bg-black text-white/60 text-[11px] border border-white/10">Semua task selesai</div>
        ) : tasks.map((t) => {
          const isCollapsed = !!collapsedIds?.has(t.id);
          const isHidden = !!(shouldCollapseView && nextTask && t.id !== nextTask.id);
          const animStyle = isHidden || isCollapsed ? `${hide} 0.4s ease both` : getAnim(t.id);
          return (
          <div key={t.id} onClick={() => onToggleTask?.(t.id)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full border shrink-0 max-w-[280px] cursor-pointer ${t.completed ? 'opacity-50 line-through' : ''} ${isHidden ? 'opacity-0 max-h-0 pointer-events-none scale-95 !py-0 !my-0' : isCollapsed ? 'task-collapsed max-w-[140px] opacity-40 scale-[0.96]' : ''} transition-all duration-400`} style={{ background: '#232d42', borderColor: 'rgba(255,255,255,0.08)', animation: animStyle, maxHeight: isHidden ? '0px' : isCollapsed ? '28px' : undefined, overflow: isHidden || isCollapsed ? 'hidden' : undefined }}>
            ...
            <span className={`text-white text-[11px] font-bold truncate ${isCollapsed ? 'opacity-60 text-[10px]' : ''}`}>{t.user ? `${t.user}: ${t.text}` : t.text}</span>
            {isCollapsed && !isHidden && <span className="text-[8px] font-black uppercase text-white/30 ml-auto">collapsed</span>}
          </div>
        )})}
      </div>
    );
  ...
  return (
    <div className="task-minimal-theme w-full max-w-[360px] bg-[#121212] border border-white/10 rounded-2xl p-5 space-y-3" style={{ fontFamily: `'${font}', sans-serif` }}>
      <div className="flex items-center justify-between">
        <span className="text-white font-black text-[11px] uppercase tracking-widest">Tasks • {tasks.length} {shouldCollapseView ? '• next only • fade' : ''}</span>
      ...
      <div className="space-y-2 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
        {tasks.map((t) => {
          ...
          const animStyle = isHidden || isCollapsed ? `${hide} 0.4s ease both` : getAnim(t.id);
          return (
          <div key={t.id} onClick={() => onToggleTask?.(t.id)} className={`flex items-center gap-2 px-3 py-2 rounded-xl border cursor-pointer text-[12px] font-bold ${t.completed ? 'bg-white/5 border-white/5 opacity-60 completed text-gray-400' : 'bg-white/10 border-white/10 text-white'} ${isHidden ? 'opacity-0 max-h-0 pointer-events-none scale-95 !py-0 !my-0' : isCollapsed ? 'task-collapsed opacity-40 scale-[0.98] !py-1.5 !gap-1.5' : ''} transition-all duration-400`} style={{ animation: animStyle, maxHeight: isHidden ? '0px' : isCollapsed ? '32px' : undefined, overflow: isHidden || isCollapsed ? 'hidden' : undefined }}>
            ...
            <span className={`task-text truncate flex-1 ${isCollapsed ? 'opacity-60 text-[11px]' : ''}`}>{t.user ? `${t.user}: ${t.text}` : t.text}</span>
