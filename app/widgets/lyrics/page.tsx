      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pl-[240px]">
        ...
        <div className="flex-1 flex flex-col lg:flex-row min-h-0">
          ...
          <div className="flex-1 bg-[#0a0a0a] p-4 md:p-6 flex flex-col min-h-[400px]">
            ...
            <div className="flex-1 bg-black border border-white/10 rounded-2xl overflow-hidden relative shadow-2xl min-h-[380px]">
              <iframe key={iframeSrc} src={iframeSrc} className="w-full h-full border-0 bg-black" allow="autoplay" />
              <div className="absolute bottom-2 right-2 text-[9px] font-mono bg-black/60 backdrop-blur px-2 py-1 rounded-full text-white/60 border border-white/10 pointer-events-none">LRCLIB {state.lrclibEnabled ? 'ON' : 'OFF'} • {state.lyricsFontSize}px • {state.lyricsAlign}</div>
            </div>
        ...
        {showLoadPopup && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm grid place-items-center z-50 p-4" onClick={() => setShowLoadPopup(false)}>
            <div onClick={e => e.stopPropagation()} className="bg-[#161616] border border-white/10 rounded-2xl p-6 w-full max-w-[480px] space-y-4">
              ...
              <input value={loadUrl} onChange={e => setLoadUrl(e.target.value)} placeholder="https://.../widgets/lyrics/display?..." className="w-full h-10 bg-black/40 border border-white/10 rounded-xl px-3 text-sm text-white" />
