...
export default function CuteTheme({ chats, font, showAvatar, showTimestamp, anim, horizontalAnim, fontSize, bgOpacity, horizontal, inline, cuteBubbleBg, cuteResubFrom, cuteResubTo, cuteBadgeBg, cuteBadgeText, cuteNameMod, cuteNameUser }: ChatThemeProps) {
  ...
  const dur = isElegant ? '0.62s' : '0.35s';
  const animStyle = `${effectiveAnim} ${dur} cubic-bezier(0.16,1,0.3,1) both`;
  ...
  if (horizontal) {
    return (
      ...
      >
        {chats.length === 0 ? (
          <div className="px-4 py-2 rounded-full text-white/60 text-[12px] flex items-center gap-2 shrink-0 border border-white/5" style={{ background: bubbleBg }}>
          ...
          if (resub) {
            ...
            return (
              <div key={c.id} className="px-3 py-2 flex items-center gap-2 shrink-0 max-w-[320px] rounded-[12px]" style={{ background: resubGrad, animation: animStyle }}>
          ...
          return (
            <div key={c.id} className="px-3 py-2 flex items-center gap-2 shrink-0 max-w-[300px] rounded-[12px]" style={{ background: bubbleBg, animation: animStyle, opacity: bgOpacity / 100 }}>
  ...
  if (inline) {
    return (
      <div className="cute-theme w-full max-w-[420px] flex flex-col gap-3 p-1" style={{ fontFamily: `'Nunito','Quicksand','${font}', sans-serif`, fontSize: `${fontSize}px`, background: 'transparent' }}>
        {chats.length === 0 ? (
          <div className="px-3.5 py-3 text-white/60 text-[13px] rounded-[12px] flex items-center gap-2" style={{ background: bubbleBg }}>
            <span className="w-2 h-2 rounded-full bg-white/20 animate-pulse" /> Menunggu chat…
          </div>
        ) : chats.map((c) => {
          const badge = getBadge(c.platform, c.nickname, c.comment);
          const resub = isResub(c.comment);
          if (resub) {
            return (
              <div key={c.id} className="px-3.5 py-3 flex items-center gap-2.5 rounded-[12px]" style={{ background: resubGrad, animation: animStyle }}>
                <span className="role-badge" style={{ background: badgeBg, color: '#a3b2f8' }}>RESUB</span>
          ...
          return (
            <div key={c.id} className="px-3.5 py-2.5 flex items-center gap-2 rounded-[12px]" style={{ background: bubbleBg, animation: animStyle, opacity: bgOpacity / 100 }}>
  ...
  return (
    <div className="cute-theme w-full max-w-[380px] flex flex-col gap-3 p-1" style={{ fontFamily: `'Nunito','Quicksand','${font}', sans-serif`, fontSize: `${fontSize}px`, background: 'transparent' }}>
      {chats.length === 0 ? (
        <div className="px-3.5 py-3 text-white/60 text-[13px] rounded-[12px] flex items-center gap-2" style={{ background: bubbleBg }}>
          <span className="w-2 h-2 rounded-full bg-white/20 animate-pulse" /> Menunggu chat… hubungkan TikTok / Streamer.bot di Dock
        </div>
      ) : chats.map((c) => {
        const badge = getBadge(c.platform, c.nickname, c.comment);
        const resub = isResub(c.comment);
        ...
        if (resub) {
          ...
          return (
            <div key={c.id} className="flex flex-col items-start gap-1" style={{ animation: animStyle }}>
